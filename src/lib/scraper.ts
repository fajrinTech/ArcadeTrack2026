...
import { getSkillBadges, normalizeProfileUrl } from './db';

export interface ScrapedBadge {
  badge_name: string;
...
export async function scrapeProfile(profileUrl: string): Promise<ScrapeResult> {
  if (!profileUrl) {
    throw new Error('URL profil wajib diisi.');
  }

  const normalizedUrl = normalizeProfileUrl(profileUrl);
  if (!normalizedUrl) {
    throw new Error('Format URL tidak valid. Gunakan format: https://www.skills.google/public_profiles/uuid');
  }
  ...
  if (response.redirected && (response.url === 'https://www.skills.google/' || response.url === 'https://www.skills.google')) {
    throw new Error('Profil tidak ditemukan atau disetel ke Privat. Silakan ubah pengaturan profil Anda menjadi Publik.');
  }
